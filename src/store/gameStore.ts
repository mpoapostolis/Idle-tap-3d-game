...
export const useGameStore = create<GameState>((set) => ({
  ...
  comboMultiplier: 1,
  helpers: [
    { id: 1, name: 'Apprentice', damage: 1, cost: 10, count: 0, color: '#4ade80' },
    { id: 2, name: 'Warrior', damage: 5, cost: 50, count: 0, color: '#f87171' },
    { id: 3, name: 'Mage', damage: 20, cost: 200, count: 0, color: '#818cf8' },
    { id: 4, name: 'Assassin', damage: 50, cost: 1000, count: 0, color: '#fb923c' },
    { id: 5, name: 'Dragon Knight', damage: 200, cost: 5000, count: 0, color: '#f472b6' },
    { id: 6, name: 'Arch Mage', damage: 1000, cost: 25000, count: 0, color: '#c084fc' },
    { id: 7, name: 'Titan Slayer', damage: 5000, cost: 100000, count: 0, color: '#fbbf24' },
  ],

  addGold: (amount) => set((state) => ({ gold: state.gold + amount })),

  upgradeClickDamage: () => set((state) => {
  ...
  buyHelper: (helperId) => set((state) => {
    ...
    const updatedHelpers = state.helpers.map((h) =>
      h.id === helperId ? { ...h, count: h.count + 1, cost: Math.floor(h.cost * 1.5) } : h
    );
  ...
  damageMonster: (damage) => set((state) => {
    ...
    if (newHp <= 0) {
      ...
      const goldEarned = Math.floor(baseGold * goldMultiplier * (1 + comboBonus));
